...
#         try:
#             insert_query = '''
#                 INSERT INTO adet_user (first_name, middle_name, last_name, address, email_address, contact_number, password)
...
#         cursor = conn.cursor()
#         cursor.execute('SELECT id, first_name FROM adet_user WHERE email = %s AND password = %s', (email_address, password))
...
def registration():
    if request.method == 'POST':
        ...
        last_name = request.form['last-name']
        address = request.form['address']
        email = request.form['email']  # Changed variable name here
        contact_number = request.form['contact-number']
        password = encrypt_password(request.form['password'])  
...
def login():
    if request.method == 'POST':
        email = request.form['email']  # Keep this as is since it's consistent with registration
        password = encrypt_password(request.form['password'])  
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT id, first_name FROM adet_user WHERE email = %s AND password = %s', (email, password))  # Changed to email_address here
        user = cursor.fetchone()
...
def dashboard():
    if 'loggedin' in session:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT first_name, middle_name, last_name, address, email, contact_number FROM adet_user WHERE id = %s', (session['id'],))  # Ensure email_address is used
        user_details = cursor.fetchone()
